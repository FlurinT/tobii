...
namespace TobiiProject {
    static class Program {
        // latest Gaze positions with global scope
        static public int gazeX = 0;
        static public int gazeY = 0;

        // struct to draw a Rectangle
        ...
        static void Main () {
            ...
            // Red circle form to highlight gaze
            Form circleForm;
            ...
            Hook.GlobalEvents ().OnCombination (new Dictionary<Combination, Action> {
                // Sets position of Cursor to Gaze position
                {
                ...
                // Send input to message without changing focus
                {
                    ...
                    () => {
                        IntPtr startingWindow = Process.GetCurrentProcess ().MainWindowHandle;
                        IntPtr gazedWindow = WindowFromPoint (new Point (xCord, yCord));

                        SetForegroundWindow (gazedWindow);

                        PostMessage (gazedWindow, WM_KEYDOWN, VK_F5, 0);
                        PostMessage (gazedWindow, WM_KEYUP, VK_F5, 0);

                        SetForegroundWindow (startingWindow);
                    }
                },
